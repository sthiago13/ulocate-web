...
export default function GestionarLugares({ isOpen, onClose }) {
  ...
  // Dummy data para simular los lugares registrados
  const [lugares, setLugares] = useState([
    { id: 1, nombre: 'Biblioteca Central', categoria: 'Áreas de Estudios' },
    { id: 2, nombre: 'Edificio A', categoria: 'Aulas' },
    { id: 3, nombre: 'Cafetería Principal', categoria: 'Áreas de Comida' },
  ]);
  ...
  const handleEdit = (lugar) => {
    // Aquí, en una implementación real, pasaríamos los datos del lugar a EditorLugar
    // Para la vista principal, solo lo abrimos.
  ...
  const handleEliminar = () => {
    setLugares(prev => prev.filter(l => l.id !== deleteConfirmation.item.id));
        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-[30px] flex flex-col gap-3 pb-[100px]">
              {filtrados.length > 0 ? (
                filtrados.map((lugar) => (
                  <div key={lugar.id} className="bg-white border border-gray-100 p-4 rounded-[16px] shadow-sm flex items-center justify-between hover:shadow-md transition-shadow group">
